...
const imageImports = import.meta.glob('../images/*.svg', { eager: true });

// Create a map of image URLs
const imageUrlMap = Object.keys(imageImports).reduce((map, path) => {
  const key = path.replace('../images/', '');
  console.log(key)
  map[key] = imageImports[path].default;
  return map;
...
const filteredDets = data.filter((job) => {
  return  filters.every((filt)=>{
        return job.role.includes(filt) || job.languages.includes(filt) ||
         job.level.includes(filt) ||  job.tools.includes(filt)
    })
        ...
        <div className='absolute -top-12  flex  flex-row shadow-[0_0_22px_0_rgba(0,0,0,0.15)]   w-[70%] lg:w-[80%] h-16 text-black bg-white rounded-lg mb-10'>
           <div className=' flex flex-row w-full'>
            ...
            {filters.map((y) => (
                    ...
                    <div className='flex flex-row  mx-2 flex-wrap items-center justify-center rounded-md bg-LightGrayishCyanTb  '>

                        <p   className= 'flex text-VeryDarkGrayishCyan  h-6 w-[70%] p-1 font-medium'>{y} </p>
                        <p onClick={FIlterRemove} className='flex  rounded-md p-1 text-white font-medium h-6 w-[30%] items-center justify-center cursor-pointer bg-VeryDarkGrayishCyan'>X</p>
                        </div>
                    
                
            ))}   </div>
            <div className=' flex w-full justify-end p-2 items-center text-center basis-1/4'>
            <p onClick={()=> RemoveAll()} className=' font-bold cursor-pointer'>Clear </p>
            </div>
...
{filteredJobs && filteredJobs.map(info => (
    <div key={info.id} className=' flex flex-col w-[70%] md:w-[80%] mb-6 mt-[60px] shadow-[0_0_22px_0_rgba(0,0,0,0.15)]  bg-white rounded-lg h-fit text-center'  >
        ...
        <div className='flex flex-wrap mb-4 ml-3 md:justify-end md:w-full md:mr-3'>
            <p onClick={() => FilterAdd(info.role)} className=' m-1 px-1 rounded-sm bg-LightGrayishCyanTb text-sm text-PrimaryDesaturatedDarkCyan font-semibold cursor-pointer'>{info.role}</p>
            <p onClick={()=> FilterAdd(info.level)} className=' m-1 px-1 rounded-sm bg-LightGrayishCyanTb text-PrimaryDesaturatedDarkCyan font-semibold text-sm cursor-pointer'>{info.level}</p>
        
            {info?.languages?.map( f => (
                <p onClick={ () => FilterAdd(f)} className='m-1 rounded-sm px-1 bg-LightGrayishCyanTb text-PrimaryDesaturatedDarkCyan font-semibold text-sm cursor-pointer'> {f}</p>
            ))}
         {info?.tools?.map( f => (
                <p onClick={() =>FilterAdd(f)} className='m-1 rounded-sm px-1 bg-LightGrayishCyanTb text-PrimaryDesaturatedDarkCyan font-semibold text-sm cursor-pointer'> {f}</p>
            ))}
